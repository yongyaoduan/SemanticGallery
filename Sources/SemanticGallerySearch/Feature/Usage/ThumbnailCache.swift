...
public actor ThumbnailCache {
    ...
    private static func loadImage(
        ...
        do {
            ...
            let image = NSImage(
                ...
                size: displaySize(for: representation.cgImage, requestedSize: size)
            )
    ...
    private func cacheKey(for item: SearchAsset, size: CGSize, scale: CGFloat) -> String {
        "\(item.assetID)-\(Int(size.width))x\(Int(size.height))@\(Int(scale * 100))"
    }

    static func displaySize(for image: CGImage, requestedSize: CGSize) -> NSSize {
        let sourceSize = CGSize(width: image.width, height: image.height)
        guard sourceSize.width > 0, sourceSize.height > 0 else {
            return NSSize(width: requestedSize.width, height: requestedSize.height)
        }

        guard requestedSize.width > 0, requestedSize.height > 0 else {
            return NSSize(width: sourceSize.width, height: sourceSize.height)
        }

        let widthScale = requestedSize.width / sourceSize.width
        let heightScale = requestedSize.height / sourceSize.height

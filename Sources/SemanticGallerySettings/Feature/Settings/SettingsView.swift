...
public struct SettingsView: View {
    ...
    public var body: some View {
        ...
        .alert(
            ...
            message: {
                Text(libraryState.privateAdaptationNotice ?? "")
            }
        )
    }
        ...
        case .indexing:
            return SettingsStatusPresentation(
                title: StatusTitleFormatter.title(
                    for: statusState.status,
                    selectedFolderExists: libraryState.selectedFolder != nil,
                    supportedImageCount: libraryState.supportedImageCount,
                    searchableImageCount: libraryState.searchableImageCount
                ),
                message: libraryState.searchIssue,
                activity: activityPresentation(
                    snapshot: StatusActivityMeter.folderPreparation(libraryState.folderPreparationProgress, now: now),
                    now: now
                )
            )
        case .training:
            return SettingsStatusPresentation(
                ...
                message: libraryState.searchIssue,
                activity: activityPresentation(
                    snapshot: StatusActivityMeter.privateAdaptation(libraryState.privateAdaptationProgress, now: now),
                    now: now
                )
            )
    ...
    private var privateAdaptationAlertBinding: Binding<Bool> {
        Binding(
            get: { libraryState.privateAdaptationNotice != nil },
            set: { isPresented in
    ...
    private var privateAdaptationAlertTitle: String {
        if let supportedImageCount = libraryState.supportedImageCount,

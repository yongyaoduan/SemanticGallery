...
func controllerStartsReadyWithoutFolderWhenInstallIsComplete() async throws {
    ...
    let suiteName = "GalleryControllerTests.\(UUID().uuidString)"
...
func controllerSelectingFolderBuildsWorkspaceResults() async throws {
    ...
    let status = await MainActor.run { controller.statusState.status }
    let selectedFolder = await MainActor.run { controller.libraryState.selectedFolder }
    let resultCount = await MainActor.run { controller.usageState.results.count }
    let resultNames = await MainActor.run { controller.usageState.results.map(\.filename) }
    let folderPreparationProgress = await MainActor.run { controller.libraryState.folderPreparationProgress }
    let searchableImageCount = await MainActor.run { controller.libraryState.searchableImageCount }
    let searchIssue = await MainActor.run { controller.libraryState.searchIssue }
    #expect(status == .ready)
...
func controllerKeepsVisibleResultsWhenEmbeddingGenerationSkipsImages() async throws {
    ...
    let status = await MainActor.run { controller.statusState.status }
    let resultNames = await MainActor.run { controller.usageState.results.map(\.filename) }
    let supportedImageCount = await MainActor.run { controller.libraryState.supportedImageCount }
    let searchableImageCount = await MainActor.run { controller.libraryState.searchableImageCount }
    let searchIssue = await MainActor.run { controller.libraryState.searchIssue }
...
func controllerDeletingSelectionMovesFilesOutOfWorkspaceResults() async throws {
    ...
    let removedPath = try #require(
        await MainActor.run { controller.usageState.results.first?.absolutePath }
    )
    let removedAssetID = try #require(
        await MainActor.run { controller.usageState.results.first?.assetID }
    )
    await MainActor.run {
        controller.toggleSelection(resultID: removedAssetID)
        controller.deleteSelectedResults()
    }
    try await waitForCondition("workspace results to refresh after delete") {
        await MainActor.run { controller.usageState.results.count == 1 }
    }
    let database = try LibraryDatabase.open(at: paths.databaseURL)
    try database.migrate()
    let visible = try database.visibleFileInstances(inFolderAbsolutePath: folder.path(percentEncoded: false), limit: 10)
...
func controllerPersistsTheChosenFolderInsideApplicationSupportAndForgetsItWhenSupportRootIsRemoved() async throws {
    ...
    defaults.removePersistentDomain(forName: suiteName)
    defer { defaults.removePersistentDomain(forName: suiteName) }
    var firstController: GalleryController? = await MainActor.run {
        GalleryController(
            runtimeOptions: .init(
                paths: paths,
                defaultsSuiteName: suiteName,
    ...
    var relaunchedController: GalleryController? = await MainActor.run {
        GalleryController(
            runtimeOptions: .init(
                paths: paths,
                defaultsSuiteName: suiteName,
    ...
    try await waitForCondition("remembered folder to become ready again") {
        await MainActor.run { relaunchedController?.statusState.status == .ready }
    }
    let relaunchedStatus = await MainActor.run { relaunchedController?.statusState.status }
    let relaunchedFolder = await MainActor.run { relaunchedController?.libraryState.selectedFolder }
    let relaunchedImageCount = await MainActor.run { relaunchedController?.libraryState.supportedImageCount }
    ...
    let resetStatus = await MainActor.run { resetController.statusState.status }
    let resetFolder = await MainActor.run { resetController.libraryState.selectedFolder }
...
func controllerCreatesSupportRootWhenSelectingFolderFromAnEmptyRuntimeState() async throws {
    ...
    #expect(await MainActor.run { controller.usageState.results.map(\.filename) } == ["front.jpg"])
}
@Test
...
func controllerRebuildsTheRememberedFolderWhenTheDatabaseIsMissingOnRelaunch() async throws {
    ...
    var controller: GalleryController? = await MainActor.run {
        GalleryController(
            runtimeOptions: .init(
                paths: paths,
                defaultsSuiteName: nil,
    ...
    try await waitForCondition("remembered folder rebuild to restore results") {
        await MainActor.run { relaunchedController.usageState.results.map(\.filename) == ["front.jpg"] }
    }
    #expect(await MainActor.run { relaunchedController.statusState.status } == .ready)
    ...
    try await waitForCondition("new folder selection stays active after bootstrap finishes") {
        await MainActor.run {
            controller.statusState.status == .ready
                && controller.libraryState.selectedFolder?.standardizedFileURL == newFolder.standardizedFileURL
                && controller.usageState.results.map(\.filename) == ["new.jpg"]
        }
    }
    let status = await MainActor.run { controller.statusState.status }
    let selectedFolder = await MainActor.run { controller.libraryState.selectedFolder }
...
@Test
func controllerDoesNotLetAnEarlierFolderSelectionOverwriteALaterOne() async throws {
    /// Formal specification for callers:
    /// Precondition:
    ///   `selectFolder(at:)` is invoked for folders `f₁` and `f₂`, with the `f₂` call starting after `f₁`
    ///   but before `f₁` finishes.
    /// Postcondition:
    ///   Once both calls finish, the visible workspace state equals the state for `f₂`.
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let paths = AppPaths(root: root)
    let firstFolder = root.appending(path: "First")
    let secondFolder = root.appending(path: "Second")
    try FileManager.default.createDirectory(at: firstFolder, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: secondFolder, withIntermediateDirectories: true)
    try Data("first".utf8).write(to: firstFolder.appending(path: "first.jpg"))
    try Data("second".utf8).write(to: secondFolder.appending(path: "second.jpg"))
    let embeddingService = DelayedOutcomeEmbeddingService(
        imageVectorsByBytes: [
            "first": [1.0, 0.0],
            "second": [0.0, 1.0],
    ...
    let firstSelection = Task {
        await controller.selectFolder(at: firstFolder)
    }
    try await Task.sleep(nanoseconds: 50_000_000)
    await controller.selectFolder(at: secondFolder)
    _ = await firstSelection.result

    try await waitForCondition("later folder selection stays active after the earlier task finishes") {
        await MainActor.run {
            controller.statusState.status == .ready
                && controller.libraryState.selectedFolder?.standardizedFileURL == secondFolder.standardizedFileURL
                && controller.usageState.results.map(\.filename) == ["second.jpg"]
        }
    }
}
...
func controllerSelectingTheSameFolderAgainReindexesNewFiles() async throws {
    ...
    await controller.selectFolder(at: folder)
    let filenames = await MainActor.run { controller.usageState.results.map(\.filename).sorted() }
    let supportedImageCount = await MainActor.run { controller.libraryState.supportedImageCount }
...
func controllerObservedFolderChangesRefreshVisibleResults() async throws {
    ...
    try await waitForCondition("observed folder changes to add a visible result") {
        await MainActor.run {
            controller.usageState.results.map(\.filename).sorted() == ["back.jpg", "front.jpg"]
                && controller.libraryState.supportedImageCount == 2
        }
    }
    ...
    try await waitForCondition("observed folder changes to remove a visible result") {
        await MainActor.run {
            controller.usageState.results.map(\.filename) == ["back.jpg"]
                && controller.libraryState.supportedImageCount == 1
        }
    }
}
@Test
func controllerSearchingSimilarFromPreviewUsesTheCurrentImage() async throws {
    ...
    let previewItem = try #require(
        await MainActor.run { controller.usageState.results.first }
    )
    await MainActor.run {
        controller.openPreview(resultID: previewItem.id)
        controller.searchSimilarToPreviewItem()
    }
    try await waitForCondition("preview similar search results") {
        await MainActor.run {
            controller.usageState.previewResultID == nil
                && controller.usageState.results.first?.filename == previewItem.filename
                && controller.usageState.pastedImageData != nil
        }
    }
...
func controllerDeletingPreviewMovesTheCurrentImageToTrash() async throws {
    ...
    await MainActor.run {
        controller.openPreview(resultID: previewItem.id)
        controller.deletePreviewResult()
    }
    try await waitForCondition("preview delete refreshes results") {
        await MainActor.run {
            controller.usageState.previewResultID == nil
                && controller.usageState.results.count == 1
        }
    }
    let database = try LibraryDatabase.open(at: paths.databaseURL)
    try database.migrate()
...
func controllerReusesStoredEmbeddingsAndRunsImageVectorSearch() async throws {
    ...
    #expect(
        await MainActor.run { controller.usageState.results.map(\.filename) } == [
            "duplicate.jpg",
            "one.jpg",
            "two.png",
        ]
    ...
    await MainActor.run {
        controller.usageState.queryText = ""
        controller.usageState.pastedImageData = Data("query-image".utf8)
        controller.runSearch()
    }
    try await waitForCondition("semantic image search results") {
        await MainActor.run { controller.usageState.results.first?.filename == "duplicate.jpg" }
    }
    let semanticResults = await MainActor.run { controller.usageState.results.map(\.filename) }
...
func controllerFolderPreparationReportsLiveProgressFromRealCounts() async throws {
    ...
    try await waitForCondition("folder progress shows scan and reuse counts", timeoutNanoseconds: 5_000_000_000) {
        await MainActor.run {
            let progress = controller.libraryState.folderPreparationProgress
            guard
    ...
    let status = await MainActor.run { controller.statusState.status }
    let results = await MainActor.run { controller.usageState.results.map(\.filename) }
    #expect(status == .ready)
    ...
    try await waitForCondition("selected folder becomes visible during preparation", timeoutNanoseconds: 2_000_000_000) {
        await MainActor.run {
            controller.libraryState.selectedFolder == folder
                && controller.libraryState.folderPreparationProgress.isEmpty == false
                && controller.statusState.status == .indexing
        }
    }
    await selectTask.value
    let finalResults = await MainActor.run { controller.usageState.results.map(\.filename) }
...
func controllerCompletesFastFolderPreparationWithoutArtificialDelay() async throws {
    ...
    #expect(elapsed < 1.0)
    #expect(await MainActor.run { controller.statusState.status } == .ready)
    #expect(await MainActor.run { controller.libraryState.selectedFolder } == folder)
    #expect(await MainActor.run { controller.libraryState.folderPreparationProgress.isEmpty })
}
@Test
...
func controllerTextSearchKeepsExactFilenameMatchesAheadOfSemanticFallback() async throws {
    ...
    await controller.selectFolder(at: folder)
    await MainActor.run {
        controller.usageState.queryText = "sample-1"
        controller.runSearch()
    }
    try await waitForCondition("filename search results") {
        await MainActor.run { controller.usageState.results.isEmpty == false }
    }
    let results = await MainActor.run { controller.usageState.results.map(\.filename) }
...
func controllerCachesVectorQueriesAcrossRepeatedSearches() async throws {
    ...
    await controller.selectFolder(at: folder)
    await MainActor.run {
        controller.usageState.queryText = "warm evening"
        controller.runSearch()
    }
    try await waitForCondition("first semantic text search results") {
        await MainActor.run { controller.usageState.results.first?.filename == "forest.jpg" }
    }
    await MainActor.run {
        controller.runSearch()
    }
    ...
    try await waitForCondition("first image semantic search results") {
        await MainActor.run { controller.usageState.results.first?.filename == "river.jpg" }
    }
    await MainActor.run {
        controller.runSearch()
    }
...
@Test
func controllerSearchWithoutSelectedFolderKeepsInputAndClearsVisibleResults() async throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let paths = AppPaths(root: root)
    ...
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let embeddingService = RecordingEmbeddingService(
        imageVectorsByBytes: ["query-image": [1.0, 0.0]],
        textVectors: ["orphan query": [0.0, 1.0]]
    )
    let controller = await MainActor.run {
        GalleryController(
            runtimeOptions: .init(
    ...
    await MainActor.run {
        controller.usageState.queryText = "orphan query"
        controller.usageState.pastedImageData = Data("query-image".utf8)
        controller.usageState.results = [
            SearchAsset(
                fileInstanceID: 41,
                assetID: 41,
                absolutePath: "/tmp/orphan.jpg",
                thumbnailPath: nil
            )
        ]
        controller.usageState.selectedResultIDs = [41]
        controller.runSearch()
    }
    try await waitForCondition("search without a folder clears stale results") {
        await MainActor.run {
            controller.usageState.results.isEmpty
                && controller.usageState.selectionCount == 0
        }
    }
    let preservedState = await MainActor.run {
        (
            queryText: controller.usageState.queryText,
            pastedImageData: controller.usageState.pastedImageData,
            status: controller.statusState.status
        )
    }
    #expect(preservedState.queryText == "orphan query")
    #expect(preservedState.pastedImageData == Data("query-image".utf8))
    #expect(preservedState.status == .readyWithoutFolder)
    #expect(await embeddingService.textEncodeCount() == 0)
    #expect(await embeddingService.pastedImageEncodeCount() == 0)
}
@Test
func controllerLogsWhenSemanticSearchHasNoEmbeddingsAvailable() async throws {
    ...
    try await waitForCondition("semantic search without embeddings") {
        await MainActor.run { controller.usageState.isSearching == false }
    }
...
@Test
func controllerStartingPrivateAdaptationWithoutSelectedFolderShowsNotice() async throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let paths = AppPaths(root: root)
    ...
    let state = await MainActor.run {
        (
            notice: controller.libraryState.privateAdaptationNotice,
            status: controller.statusState.status
        )
    }
    #expect(state.notice == "Choose a folder in Settings before starting private adaptation.")
    #expect(state.status == .readyWithoutFolder)
    #expect(await adaptationTrainer.runCount() == 0)
}
@Test
func controllerUninstallClearsInstalledStateAndReturnsToInstallRequired() async throws {
    ...
    let clearedState = await MainActor.run {
        (
            status: controller.statusState.status,
            selectedFolder: controller.libraryState.selectedFolder,
            supportedImageCount: controller.libraryState.supportedImageCount,
            searchableImageCount: controller.libraryState.searchableImageCount,
            activeEncoderVersion: controller.libraryState.activeEncoderVersion,
            searchIssue: controller.libraryState.searchIssue,
            resultsCount: controller.usageState.results.count,
            selectionCount: controller.usageState.selectedResultIDs.count,
            pastedImageData: controller.usageState.pastedImageData
        )
    }
...
func controllerRunsPrivateAdaptationAndSwitchesToTheFolderSpecificEncoder() async throws {
    ...
    try await waitForCondition("private adaptation finishes") {
        await MainActor.run {
            controller.libraryState.activeEncoderVersion == "stage2-privatealbum-abcdef"
                && controller.statusState.status == .ready
        }
    }
    let status = await MainActor.run { controller.statusState.status }
    let activeEncoderVersion = await MainActor.run { controller.libraryState.activeEncoderVersion }
    #expect(status == .ready)
    #expect(activeEncoderVersion == "stage2-privatealbum-abcdef")
    #expect(await adaptationTrainer.runCount() == 1)
}
@Test
func controllerDeletingSharedImageOnlyRemovesTheSelectedFolderInstance() async throws {
    let root = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)
    try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
    defer { try? FileManager.default.removeItem(at: root) }
    let paths = AppPaths(root: root)
    let coordinator = InstallCoordinator(paths: paths, downloader: ArtifactDownloader.stubbed)
    _ = try await coordinator.prepare()
    let firstFolder = root.appending(path: "Library-A")
    let secondFolder = root.appending(path: "Library-B")
    try FileManager.default.createDirectory(at: firstFolder, withIntermediateDirectories: true)
    try FileManager.default.createDirectory(at: secondFolder, withIntermediateDirectories: true)
    let firstURL = firstFolder.appending(path: "shared.jpg")
    let secondURL = secondFolder.appending(path: "shared.jpg")
    ...
    defer { defaults.removePersistentDomain(forName: suiteName) }
    let embeddingService = RecordingEmbeddingService(
        imageVectorsByBytes: ["shared": [1.0, 0.0]],
        textVectors: [:]
    )
    let controller = await MainActor.run {
        GalleryController(
    ...
    await controller.selectFolder(at: secondFolder)
    await controller.selectFolder(at: firstFolder)
    let sharedRecordID = try #require(await MainActor.run { controller.usageState.results.first?.id })
    await MainActor.run {
        controller.enterSelectionMode()
        controller.toggleSelection(resultID: sharedRecordID)
        controller.deleteSelectedResults()
    }
    try await waitForCondition("shared image deletion refreshes the selected folder") {
        await MainActor.run {
            controller.libraryState.selectedFolder == firstFolder
                && controller.usageState.results.isEmpty
        }
    }
    await controller.selectFolder(at: secondFolder)
    let remainingNames = await MainActor.run { controller.usageState.results.map(\.filename) }
    let database = try LibraryDatabase.open(at: paths.databaseURL)
    try database.migrate()
    let firstFolderVisible = try database.visibleFileInstances(
        inFolderAbsolutePath: firstFolder.path(percentEncoded: false),
        limit: 10
    )
    let secondFolderVisible = try database.visibleFileInstances(
        inFolderAbsolutePath: secondFolder.path(percentEncoded: false),
        limit: 10
    )
    let secondFolderEmbeddings = try database.embeddedFiles(
        inFolderAbsolutePath: secondFolder.path(percentEncoded: false),
        encoderVersion: "stage1"
    )
    #expect(FileManager.default.fileExists(atPath: firstURL.path) == false)
    #expect(FileManager.default.fileExists(atPath: secondURL.path) == true)
    #expect(firstFolderVisible.isEmpty)
    #expect(secondFolderVisible.map(\.relativePath) == ["shared.jpg"])
    #expect(secondFolderEmbeddings.count == 1)
    #expect(remainingNames == ["shared.jpg"])
    #expect(await embeddingService.imageEncodeCount() == 1)
}
@Test
func controllerEnteringSelectionModeClearsPreviewAndMetadata() async throws {
    ...
    await MainActor.run {
        controller.openPreview(resultID: firstAssetID)
        controller.togglePreviewMetadata()
        controller.enterSelectionMode()
    }
    let previewResultID = await MainActor.run { controller.usageState.previewResultID }
    let isPreviewMetadataVisible = await MainActor.run { controller.usageState.isPreviewMetadataVisible }
    let isSelectionModeEnabled = await MainActor.run { controller.usageState.isSelectionModeEnabled }
    let selectionCount = await MainActor.run { controller.usageState.selectionCount }
    #expect(previewResultID == nil)
...
func controllerLeavingSelectionModeClearsSelectionAndOpeningPreviewLeavesBrowseState() async throws {
    ...
    await MainActor.run {
        controller.enterSelectionMode()
        controller.toggleSelection(resultID: firstAssetID)
    }
    let selectedCountBeforeLeaving = await MainActor.run { controller.usageState.selectionCount }
    #expect(selectedCountBeforeLeaving == 1)
    await MainActor.run {
        controller.leaveSelectionMode()
        controller.openPreview(resultID: firstAssetID)
    }
    let selectedCountAfterLeaving = await MainActor.run { controller.usageState.selectionCount }
    let isSelectionModeEnabled = await MainActor.run { controller.usageState.isSelectionModeEnabled }
    let previewResultID = await MainActor.run { controller.usageState.previewResultID }
    #expect(selectedCountAfterLeaving == 0)
    #expect(isSelectionModeEnabled == false)
    #expect(previewResultID == firstAssetID)
}

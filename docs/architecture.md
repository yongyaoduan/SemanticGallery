...
SemanticGallery works in two stages. The offline stage builds a gallery index from local files. The online stage serves search results from that index.

## Model

![Model structure](assets/readme/model-structure.png)

The text tower encodes the query. The vision tower encodes each gallery image. Both outputs are L2-normalized into the same embedding space. In practice, that means online retrieval is one text forward pass plus vector search over precomputed gallery embeddings.
...
4. If a metadata manifest is present, the runtime applies caption substring and token-match heuristics from that manifest before final ranking. That step can reshuffle close candidates.
...
When the user deletes an image from the web UI, the app removes the file from the gallery, updates the file-backed index, rewrites the metadata manifest if one is present, and updates the incremental gallery-sync state files. The next search sees the same state as the local folder. The delete is permanent.

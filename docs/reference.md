...
| Variable | Details |
| --- | --- |
| `GALLERY_PATH` | Required. Set an absolute path to the local image folder that SemanticGallery should adapt, index, and serve. |
| `STAGE1_WEIGHTS` | Default: local Stage 1 if present, otherwise the published checkpoint. Unset: `quickstart.sh` uses the local Stage 1 checkpoint when present and falls back to the published checkpoint otherwise. Set a `weights.safetensors` path: start Stage 2 from that file. |
...
| `CONFIG_OUTPUT` | Default: `deployment/search_config_gallery_mlx.json`. Set a JSON file path to change where the generated search config is written. |
| `METADATA_MANIFEST` | Default: `datasets/private_gallery_local/full_manifest.jsonl`. Unset: disable manifest-backed metadata display and the small metadata-based ranking boost. Set a JSONL path: load weak captions from that file for metadata display and ranking boost. |
...
| `LOG_FILE` | Default: `logs/runtime/semanticgallery_<port>.log`. Set a file path to override the startup log location. |
| `PID_FILE` | Default: `logs/runtime/semanticgallery_<port>.pid`. Set a file path to override the PID file location. |
...
| Variable | Details |
| --- | --- |
| `GALLERY_PATH` | Required. Set an absolute path to the local image folder that should be indexed and served. |
| `CONFIG_OUTPUT` | Default: `deployment/search_config_gallery_mlx.json`. Set a JSON file path to change where the generated search config is written. |
| `METADATA_MANIFEST` | Default: `datasets/private_gallery_local/full_manifest.jsonl`. Unset: disable manifest-backed metadata display and the small metadata-based ranking boost. Set a JSONL path: load weak captions from that file for metadata display and ranking boost. |
| `HOST` | Default: `127.0.0.1`. `127.0.0.1`: bind only the local machine. `0.0.0.0`: bind every interface and expose the app to the local network. Any other address: bind only that specific interface. |
| `PORT` | Default: `36168`. Set any unused TCP port to change the web URL and the runtime log and PID filenames. |
| `MODEL_PRECISION` | Default: `bfloat16`. `bfloat16`: normal fast path for gallery encoding and query encoding. `float32`: slower and uses more memory, but is the conservative full-precision option. |
| `MODEL_WEIGHTS` | Default: latest local adaptation if present, otherwise Stage 1. Unset: deployment prefers the latest local adaptation weights and falls back to Stage 1 when no local adaptation exists. Set a `weights.safetensors` path: use that file for both gallery encoding and query encoding. |
...
| Variable | Details |
| --- | --- |
| `PRIVATE_GALLERY_PATH` | Required unless the local JSONL files already exist. Set an absolute path to the user's gallery to build `full_manifest.jsonl` and `private_adapt_data.jsonl`. |
...
| `STAGE1_WEIGHTS` | Default: local Stage 1 if present, otherwise the published checkpoint. Unset: `adapt_best.sh` uses the local Stage 1 checkpoint when present and falls back to the published checkpoint otherwise. Set a `weights.safetensors` path: start Stage 2 from that file. |
